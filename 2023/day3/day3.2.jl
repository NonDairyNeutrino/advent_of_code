...
struct Symbol
    ...
    isGear :: Bool
    gearRatio :: Int
    function Symbol(position :: CartesianIndex, value :: Char, partVector :: Vector{Part})
        isGear = value == '*' && length(partVector) == 2
        gearRatio = prod(part.value for part in partVector)
        return new(position, value, partVector, isGear, gearRatio)
    end
end
...
function getSymbolParts(charMatrix :: Matrix{Char}, symbolPosition :: CartesianIndex{2}) :: Vector{Part}
    ...
    for adjacentIndex in adjacentIndexMatrix
        ...
        if part ∉ symbolPartVector && part !== nothing
...
function main()
    ...
    return sum(symbol.gearRatio for symbol in symbolVector if symbol.isGear)
end
